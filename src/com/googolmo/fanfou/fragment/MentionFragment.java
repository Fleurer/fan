...
//        getListView().scrollTo(scrolledIndex, scrolledTop);
    ...
    public void onStop() {
        NLog.d(TAG, "onStop()");
        super.onStop();
        getProvider().setHomeMTScrollPostion(scrolledIndex, scrolledTop);
    }
    ...
    public Loader<List<Status>> onCreateLoader(int i, Bundle bundle) {
        ...
        if (i == 0) {
            return new DBLoader(getActivity(), getProvider(), DB.STATUS_TYPE_MENTIONS, null);
        }
//        else if (i == 1) {
    ...
    public void onLoadFinished(Loader<List<Status>> listLoader, List<Status> statuses) {
        ...
        if (listLoader.getId() == 0) {
            if (statuses.size() < 1) {
                ...
                int p = getProvider().getHomeMTLoadmoreMark(mCurrentUserId);
                if (p > 0) {
                    Status s = new Status();
                    s.setRawid(0);
                    this.mStatuses.add(p, s);
//                    getProvider().addHomeMTLoadmoreMark(mCurrentUserId, p);
                }
                isSetPostion = true;
            }
            ...
            if (mLoadIndex == 0) {
                ...
                if (statuses.size() < getProvider().getLoadCount()) {
                    ...
                    this.mStatuses.addAll(0, statuses);
                    getProvider().addHomeMTLoadmoreMark(mCurrentUserId, statuses.size());
                ...
                if (this.mStatuses.size() > getProvider().getMaxCount()) {
                    this.mStatuses.clear();
                    getProvider().addHomeMTLoadmoreMark(mCurrentUserId, 0);
...
//                this.mStatuses.remove(mLoadIndex);
                ...
                if (this.mStatuses.size() + statuses.size() >= getProvider().getMaxCount()) {
                    ...
                    if (statuses.size() == getProvider().getLoadCount()) {
                        ...
                        getProvider().addHomeMTLoadmoreMark(mCurrentUserId, mLoadIndex + statuses.size());
                    }
                }
            ...
            new Thread(){
                ...
                public void run() {
                    super.run();
                    getProvider().getDB().addStatuses(mStatuses, mProvider.getCurrentUserId(), DB.STATUS_TYPE_MENTIONS);
                }
            }.start();
        }
